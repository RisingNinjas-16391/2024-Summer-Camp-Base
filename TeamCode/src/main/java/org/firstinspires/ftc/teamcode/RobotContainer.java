...
public class RobotContainer {
    ...
    //private final GamepadButton m_outtake2Position;
    //private final GamepadButton m_intakePosition;
    //private final GamepadButton m_rampPosition;
    ...
    public RobotContainer(HardwareMap hwMap, Gamepad gamepad1, Gamepad gamepad2, int autoNum){
       ...
       /* m_outtakePosition = new GamepadButton(m_driverController, GamepadKeys.Button.X);
        m_outtake2Position = new GamepadButton(m_driverController, GamepadKeys.Button.Y);
        m_intakePosition = new GamepadButton(m_driverController, GamepadKeys.Button.A);
        m_rampPosition = new GamepadButton(m_driverController, GamepadKeys.Button.B); */
...
//                m_pivotSubsystem, () -> (m_driverController.getButton(GamepadKeys.Button.X) ? 1 : m_driverController.getButton(GamepadKeys.Button.B) ? -1 : 0)));
    ...
    public void configureButtonBindings() {
       /* m_outtakePosition.whenPressed(new PivotCommand(m_pivotSubsystem, 140));
        m_outtake2Position.whenPressed(new PivotCommand(m_pivotSubsystem,90));
        m_intakePosition.whenPressed(new PivotCommand(m_pivotSubsystem, 0));
        m_rampPosition.whenPressed(new PivotCommand(m_pivotSubsystem,30)); */
...
//        m_pivotDown.whenInactive(new PivotPowerCommand(m_pivotSubsystem, () -> 0));
    ...
    private void setAutoCommands(int chooser) {
        switch (chooser) {
            case 1:
                new BlueAutoCommand(m_driveSubsystem, m_pivotSubsystem).schedule();
                break;
            case 2:
                new RedAutoCommand(m_driveSubsystem, m_pivotSubsystem).schedule();
                break;
        }
